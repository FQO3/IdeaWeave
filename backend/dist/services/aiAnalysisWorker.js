...
exports.aiAnalysisWorker = exports.AIAnalysisWorker = void 0;
...
class AIAnalysisWorker {
    ...
    async processBacklog() {
        ...
        try {
            // 首先检查数据库表是否有AI分析字段
            const tableInfo = await db_1.db.select({
                id: schema_1.ideas.id,
                content: schema_1.ideas.content,
                userId: schema_1.ideas.userId
            })
                .from(schema_1.ideas)
                .limit(1);
            // 如果查询成功，继续处理积压任务
            const pendingIdeas = await db_1.db.select()
                .from(schema_1.ideas)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ideas.aiAnalysisStatus, 'pending'), (0, drizzle_orm_1.lt)(schema_1.ideas.aiAnalysisAttempts, this.maxRetries)))
                .limit(10); // 每次最多处理10个
    ...
    async processTask(task) {
        ...
        await db_1.db.update(schema_1.ideas)
            ...
            aiAnalysisAttempts: (0, drizzle_orm_1.sql) `${schema_1.ideas.aiAnalysisAttempts} + 1`,
            lastAnalysisAttempt: new Date()
        })
            .where((0, drizzle_orm_1.eq)(schema_1.ideas.id, task.ideaId));
        try {
            // 获取用户现有的笔记用于AI分析
            const existingIdeas = await db_1.db.select()
                .from(schema_1.ideas)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ideas.userId, task.userId), (0, drizzle_orm_1.eq)(schema_1.ideas.aiAnalysisStatus, 'completed')))
                .limit(50);
            // 调用DeepSeek API进行分析
            const analysis = await deepseek_1.deepSeekService.analyzeIdea(task.content, existingIdeas, task.userId);
            // 更新笔记，包含AI分析结果
            await db_1.db.update(schema_1.ideas)
                .set({
                ...
                const tagPromises = analysis.tags.map(async (tagData) => {
                    try {
                        ...
                        const [existingTag] = await db_1.db.select()
                            .from(schema_1.tags)
                            .where((0, drizzle_orm_1.eq)(schema_1.tags.name, tagData.name))
                            .limit(1);
                        let tagId;
                        if (existingTag) {
            ...
            if (analysis.relatedIdeas && analysis.relatedIdeas.length > 0) {
                const linkPromises = analysis.relatedIdeas.map(async (related) => {
                    // 验证相关笔记确实存在且属于该用户
                    const [existingIdea] = await db_1.db.select()
                        .from(schema_1.ideas)
                        .where((0, drizzle_orm_1.eq)(schema_1.ideas.id, related.ideaId))
                        .limit(1);
                    if (existingIdea) {
                        try {
                            await db_1.db.insert(schema_1.links).values({
                                id: (0, uuid_1.v4)(),
            ...
            // 检查是否需要重试
            const [currentIdea] = await db_1.db.select()
                .from(schema_1.ideas)
                .where((0, drizzle_orm_1.eq)(schema_1.ideas.id, task.ideaId))
                .limit(1);
    ...
    async markTaskAsFailed(ideaId, error) {
        try {
            await db_1.db.update(schema_1.ideas)
                .set({
...
exports.AIAnalysisWorker = AIAnalysisWorker;
// 创建全局实例
exports.aiAnalysisWorker = new AIAnalysisWorker();
